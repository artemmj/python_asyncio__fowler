...
async def encrypt(sema: asyncio.Semaphore, text: str) -> bytes:
...
async def main():
    ...
    sema = asyncio.Semaphore(os.cpu_count())
    s = time.time()
    tasks = [asyncio.create_task(encrypt(sema, text)) for text in text_list]
